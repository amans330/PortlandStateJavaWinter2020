...
public class Project4 {
	...
	public static void main(String[] args) {
		...
		boolean readFromTextFile = false;
		boolean readFromXMLFile = false;
		...
		optionsValues.add("-xmlFile");
		optionsValues.add("-pretty");

		// 0 arguments check
		if (args.length == 0) Util.printErrorAndExit("There were no arguments passed!! Please read the ReadMe and then proceed.");
		...
		if (args.length < 10 || args.length > 16) {
			...
			Util.printErrorAndExit("Not enough or too many arguments passed!! Please read the ReadMe and then proceed.");
		}
		...
		for (int i = 0; i < args.length; i++) {
			String str = args[i];
			if(str.charAt(0) == '-' && !(str.equals("-") && !args[i-1].equals("-pretty")) && !optionsValues.contains(str))
				Util.printErrorAndExit("Unknown command line option.");
			if (str.equals("-README")) {
				...
				if(readFromXMLFile) Util.printErrorAndExit("It is an error to specify both -textFile and -xmlFile.");
				
				// there is no argument after -textFile
				if (i + 1 >= args.length) Util.printErrorAndExit("Please pass the file name to read/write from.");
				
				readFromTextFile = true;
				path = args[i + 1];
				i++;
			} else if (str.equals("-xmlFile")) {
				if(readFromTextFile) {
					Util.printErrorAndExit("It is an error to specify both -textFile and -xmlFile.");
				}
				if (i + 1 >= args.length) { // there is no argument after -xmlFile
					Util.printErrorAndExit("Please pass the XML file name to read/write from.");
				}
				readFromXMLFile = true;
				path = args[i + 1];
				if(path.charAt(0) == '-') // there is an option argument after -xmlFile
					Util.printErrorAndExit("Please pass the XML file name to read/write from.");
				i++;
		...
		CheckDataValidity.checkArgumentsDataValidity(arguments);
		...
		if (readFromTextFile == true) {
			// append the flights mentioned in the file with the one given in the console,
...
//				e.printStackTrace();
		...
		} else if(readFromXMLFile == true) {
			// append the flights mentioned in the XML file with the one given in the console,
			// throw an error if airline names mismatch or XML errors occurs
			XmlParser parser = new XmlParser(path, airline);
			try {
				updatedAirline = (Airline) parser.parse();
			} catch (ParserException e) {
